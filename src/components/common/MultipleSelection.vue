<template>
    <div class="multiple">
        ...
        <ul class="multiple-c" v-else>
            ...
            <li
                @click="multipleClick(Number(i) + 2)"
                :class="!item.check?'vux-1px tap_events':'select vux-1px tap_events'"
        ...
        <div
            v-if="types !== 'method'"
            :class="multipleFlag.length >= 5?'btn tap_events':'btn btnSelect tap_events'"
            :style="{backgroundImage: multipleFlag.length >= 5?'url(btn_bg_Evaluation_yellow_activation.png)':'url(btn_bg_Evaluation_yellow_enabled.png)'} | PUBLIC_PATH"
            @click="multipleSure"
        >{{ btn }}</div>
        <div
            v-else
